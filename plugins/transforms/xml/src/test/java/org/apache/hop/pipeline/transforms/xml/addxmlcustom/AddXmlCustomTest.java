...
public class AddXmlCustomTest {

  private TransformMockHelper<AddXmlCustomMeta, AddXmlCustomData> transformMockHelper;

  @Before
  public void setup() throws Exception {
    HopEnvironment.init();
    XmlFieldCustom field = mock(XmlFieldCustom.class);
    when(field.getElementName()).thenReturn("ADDXML_TEST");
    when(field.isAttribute()).thenReturn(true);

    transformMockHelper =
        new TransformMockHelper<>("ADDXML_TEST", AddXmlCustomMeta.class, AddXmlCustomData.class);
    ...
    when(transformMockHelper.iTransformMeta.getOutputFields())
        .thenReturn(new XmlFieldCustom[] {field});
