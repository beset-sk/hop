...
@Transform(id = "Rapidstarttable", name = "i18n::Rapidstarttable.Name", description = "i18n::Rapidstarttable.Description", image = "rapidstart.svg", categoryDescription = "i18n:org.apache.hop.pipeline.transform:BaseTransform.Category.Output", documentationUrl = "/pipeline/transforms/rapidstarttable.html", keywords = "package,code,rapid,start")
	...
	public void check(List<ICheckResult> remarks, PipelineMeta pipelineMeta, TransformMeta transformMeta, IRowMeta prev,
		...
		if (input == null || input.length == 0) {
			remarks.add(new CheckResult(ICheckResult.TYPE_RESULT_WARNING,
					BaseMessages.getString(PKG, "PackageCodeMeta.CheckResult.NoInput"), transformMeta));
		} else {
			remarks.add(new CheckResult(ICheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "PackageCodeMeta.CheckResult.AcceptsInput"), transformMeta));
		}
